...
{
    public class GetCreditWorksByCourseIdRequestHandler :
        IRequestHandler<GetCreditWorksByCourseIdRequest, BaseQueryListResponse<GetCreditWorkDto>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IUserService _userService;
        private readonly ICurrentUserService _currentUserService;
        public GetCreditWorksByCourseIdRequestHandler(IUnitOfWork uow, IUserService userService, ICurrentUserService currentUserService)
        {
            _unitOfWork = uow;
            _userService = userService;
            _currentUserService = currentUserService;
        }
        public async Task<BaseQueryListResponse<GetCreditWorkDto>> Handle(GetCreditWorksByCourseIdRequest request, CancellationToken cancellationToken)
        {
            var response = new BaseQueryListResponse<GetCreditWorkDto>();
            try
            {
                var repository = _unitOfWork.CreditWorkRepository;
                var entities = await repository.GetCreditWorksByCourseIdAsync(request.CourseId);
                if (entities is null)
                {
